...
from .strategies.liquidity_screened_absolute_momentum_composite import (
    DEFAULT_HOLD_BARS,
...
STRATEGY_KEY = "liquidity-screened-absolute-momentum-composite"
...
def _run_id(
    *,
    ...
    return (
        f"{timestamp}__{timeframe}__lb{lookback_bars}_vw{volatility_window}_lw{liquidity_window}_top{top_k}"
        f"_h{hold_bars}_rb{rebalance_interval}_mv{_decimal_slug(min_volatility_pct)}"
        f"_am{_decimal_slug(min_momentum_pct)}_lr{_decimal_slug(liquidity_universe_ratio)}"
    )
...
def run_backtest(
    *,
    ...
    if engine == "account":
        ...
        summary["liquidity_window"] = liquidity_window
        summary["min_volatility_pct"] = min_volatility_pct
        summary["min_momentum_pct"] = min_momentum_pct
        summary["liquidity_universe_ratio"] = liquidity_universe_ratio
        ...
        summary["avg_liquidity_eligible_universe_size"] = result.avg_liquidity_eligible_universe_size
        summary["avg_selected_median_dollar_volume"] = round(float(liquidity_series.mean()), 4) if not liquidity_series.empty else None
    else:
    ...
    meta = {
        ...
        "strategy_label": "Liquidity-Screened Absolute Momentum Gated Composite",
        "engine_type": engine,
        "timeframe": timeframe,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest a liquidity-screened absolute-momentum-gated momentum + volatility composite prototype on crypto data")
    ...
    parser.add_argument("--liquidity-window", type=int, default=DEFAULT_LIQUIDITY_WINDOW)
    ...
    parser.add_argument("--liquidity-universe-ratio", type=float, default=DEFAULT_LIQUIDITY_UNIVERSE_RATIO)
    ...
    trade_frame, summary, run_dir, _ = run_backtest(
        ...
        liquidity_universe_ratio=args.liquidity_universe_ratio,
        symbols=symbols,
    ...
    for key in (
        ...
        "avg_selected_median_dollar_volume",
        "total_return_pct",
