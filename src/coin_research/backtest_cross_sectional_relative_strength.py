...
from .strategies.cross_sectional_relative_strength import (
    DEFAULT_HOLD_BARS,
...
def _run_id(*, timeframe: str, lookback_bars: int, top_k: int, hold_bars: int, rebalance_interval: int) -> str:
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    return f"{timestamp}__{timeframe}__lb{lookback_bars}_top{top_k}_h{hold_bars}_rb{rebalance_interval}"
...
def run_backtest(
    *,
    ...
    meta = {
        ...
        "strategy_label": "Cross-sectional Relative Strength",
        "engine_type": engine,
        "timeframe": timeframe,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest a minimal cross-sectional relative strength prototype on crypto data")
