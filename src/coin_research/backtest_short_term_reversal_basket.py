...
from .strategies.short_term_reversal_basket import (
    DEFAULT_BOTTOM_K,
    DEFAULT_HOLD_BARS,
...
def _run_id(
    *,
    ...
    return (
        f"{timestamp}__{timeframe}__lb{lookback_bars}_bot{bottom_k}"
        f"_h{hold_bars}_rb{rebalance_interval}_md{_decimal_slug(min_drop_pct)}"
    )
...
def run_backtest(
    *,
    ...
    if engine == "account":
        ...
        summary["min_drop_pct"] = min_drop_pct
        summary["rebalance_interval"] = rebalance_interval
        summary["bottom_k"] = bottom_k
    ...
    meta = {
        ...
        "strategy_label": "Short-term Reversal Basket",
        "engine_type": engine,
        "timeframe": timeframe,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest a minimal short-term reversal basket prototype on crypto data")
    ...
    parser.add_argument("--min-drop-pct", type=float, default=DEFAULT_MIN_DROP_PCT)
