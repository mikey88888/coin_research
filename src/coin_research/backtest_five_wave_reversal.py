...
EXIT_MODE_CHOICES = {
    "time_only": EXIT_MODE_TIME_ONLY,
    "trailing_stop": EXIT_MODE_TRAILING_STOP,
    "three_wave_exit": EXIT_MODE_THREE_WAVE,
}
...
def _run_id(*, timeframe: str, exit_mode: str, trailing_stop_pct: float | None) -> str:
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    if exit_mode == "trailing_stop":
        pct = int(round((trailing_stop_pct or DEFAULT_TRAILING_STOP_PCT) * 100))
        return f"{timestamp}__{timeframe}__{exit_mode}_{pct}"
    return f"{timestamp}__{timeframe}__{exit_mode}"
...
def run_backtest(
    *,
    ...
    target_symbols = available_symbols if symbols is None else [symbol for symbol in symbols if symbol in set(available_symbols)]
    ...
    run_id = _run_id(timeframe=timeframe, exit_mode=exit_mode, trailing_stop_pct=trailing_stop_pct)
    ...
    for symbol in target_symbols:
        ...
        if frame.empty:
            continue
        result = run_five_wave_reversal_backtest(
            frame,
            symbol=symbol,
            time_column=TIMEFRAME_CHOICES[timeframe],
            exit_mode=EXIT_MODE_CHOICES[exit_mode],
    ...
    if engine == "account":
        ...
        trade_frame = pd.DataFrame([trade.to_record() for trade in account_result.trades]).sort_values(["entry_date", "symbol"], na_position="last").reset_index(drop=True) if account_result.trades else pd.DataFrame()
    ...
    meta = {
        ...
        "params": {
            ...
            "trailing_stop_pct": trailing_stop_pct if exit_mode == "trailing_stop" else None,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest the five-wave accelerated-down reversal strategy on crypto data")
    ...
    parser.add_argument("--exit-mode", choices=sorted(EXIT_MODE_CHOICES), default="three_wave_exit")
    parser.add_argument("--trailing-stop-pct", type=float, default=DEFAULT_TRAILING_STOP_PCT)
