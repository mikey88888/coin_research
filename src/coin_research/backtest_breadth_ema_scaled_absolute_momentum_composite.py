...
from .strategies.breadth_ema_scaled_absolute_momentum_composite import (
...
def _run_id(
    *,
    ...
    return (
        ...
        f"_am{_decimal_slug(min_momentum_pct)}_bf{_decimal_slug(breadth_momentum_floor_pct)}"
        f"_bsf{_decimal_slug(breadth_scale_floor_ratio)}_be{breadth_ema_span}"
    )
...
def run_backtest(
    *,
    ...
    run_id = _run_id(
        ...
        breadth_ema_span=breadth_ema_span,
    )
    ...
    trades_per_rebalance = (
        pd.Series([len(group) for _, group in pd.DataFrame([trade.to_record() for trade in trades]).groupby("signal_confirm_date")], dtype="float64")
    ...
    if engine == "account":
        ...
        summary["breadth_momentum_floor_pct"] = breadth_momentum_floor_pct
        summary["breadth_scale_floor_ratio"] = breadth_scale_floor_ratio
        summary["breadth_ema_span"] = breadth_ema_span
        summary["rebalance_interval"] = rebalance_interval
        summary["top_k"] = top_k
        summary["universe_symbols"] = len(market_frames)
        summary["evaluated_rebalance_count"] = result.evaluated_rebalance_count
        summary["selected_rebalance_count"] = result.selected_rebalance_count
        summary["skipped_for_scale_zero_rebalance_count"] = result.skipped_for_scale_zero_rebalance_count
        summary["avg_regime_breadth_pct"] = round(float(breadth_series.mean()), 4) if not breadth_series.empty else None
        summary["avg_target_positions"] = round(float(trades_per_rebalance.mean()), 4) if not trades_per_rebalance.empty else None
        summary["avg_exposure_scale_pct"] = round(float((trades_per_rebalance / float(top_k) * 100.0).mean()), 4) if not trades_per_rebalance.empty else None
    else:
    ...
    meta = {
        ...
        "strategy_label": "Breadth-EMA-Scaled Absolute Momentum Composite",
        "engine_type": engine,
        "timeframe": timeframe,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest a breadth-EMA-scaled absolute-momentum composite prototype on crypto data")
    ...
    parser.add_argument("--breadth-momentum-floor-pct", type=float, default=DEFAULT_BREADTH_MOMENTUM_FLOOR_PCT)
    parser.add_argument("--breadth-scale-floor-ratio", type=float, default=DEFAULT_BREADTH_SCALE_FLOOR_RATIO)
    parser.add_argument("--breadth-ema-span", type=int, default=DEFAULT_BREADTH_EMA_SPAN)
    ...
    trade_frame, summary, run_dir, _ = run_backtest(
        ...
        breadth_momentum_floor_pct=args.breadth_momentum_floor_pct,
        breadth_scale_floor_ratio=args.breadth_scale_floor_ratio,
    ...
    for key in (
        ...
        "best_trade_pct",
        "worst_trade_pct",
        "avg_regime_breadth_pct",
        "avg_target_positions",
        "avg_exposure_scale_pct",
        "selected_rebalance_count",
        "skipped_for_scale_zero_rebalance_count",
        "total_return_pct",
