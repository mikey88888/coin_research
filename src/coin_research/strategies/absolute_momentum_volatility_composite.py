...
DEFAULT_MIN_MOMENTUM_PCT = 0.0
EXIT_REASON_REBALANCE = "rebalance_exit"


@dataclass(frozen=True)
class AbsoluteMomentumVolatilityCompositeTrade:
    signal_id: str
    symbol: str
    wave_start_date: Any
    wave_end_date: Any
...
@dataclass(frozen=True)
class AbsoluteMomentumVolatilityCompositeResult:
    trades: list[AbsoluteMomentumVolatilityCompositeTrade]
...
def run_absolute_momentum_volatility_composite_backtest(
    ...
    *,
...
) -> AbsoluteMomentumVolatilityCompositeResult:
    ...
    if not prepared_frames:
        return AbsoluteMomentumVolatilityCompositeResult(trades=[])
    ...
    for signal_index in range(start_index, len(timeline) - 1, rebalance_interval):
        ...
        ranking_frame = ranking_frame[ranking_frame["momentum_pct"] >= min_momentum_pct]
        ...
        for rank, (symbol, row) in enumerate(selected.iterrows(), start=1):
            ...
            trades.append(
                AbsoluteMomentumVolatilityCompositeTrade(
                    signal_id=(
                        f"{symbol}-{_format_signal_time(signal_time)}-"
                        f"lb{lookback_bars}-vw{volatility_window}-top{top_k}-h{hold_bars}-am{min_momentum_pct}-r{rank}"
                    ),
                    symbol=symbol,
    ...
    return AbsoluteMomentumVolatilityCompositeResult(trades=trades)
