...
DEFAULT_BREADTH_MOMENTUM_FLOOR_PCT = 0.0
DEFAULT_BREADTH_SCALE_FLOOR_RATIO = 0.4
EXIT_REASON_REBALANCE = "rebalance_exit"


@dataclass(frozen=True)
class BreadthScaledAbsoluteMomentumCompositeTrade:
    signal_id: str
    symbol: str
    wave_start_date: Any
    wave_end_date: Any
...
@dataclass(frozen=True)
class BreadthScaledAbsoluteMomentumCompositeResult:
    trades: list[BreadthScaledAbsoluteMomentumCompositeTrade]
    evaluated_rebalance_count: int
    selected_rebalance_count: int
    skipped_for_scale_zero_rebalance_count: int
...
def _validate_params(
    *,
    ...
    breadth_momentum_floor_pct: float,
    breadth_scale_floor_ratio: float,
) -> int:
    if lookback_bars <= 0:
    ...
    if not 0.0 <= breadth_scale_floor_ratio < 1.0:
        raise ValueError(f"breadth_scale_floor_ratio must be within [0, 1), got {breadth_scale_floor_ratio}")
    ...
    if breadth_momentum_floor_pct > min_momentum_pct:
        raise ValueError(
            "breadth_momentum_floor_pct should be <= min_momentum_pct so the breadth proxy is not stricter than the selection gate"
        )
    return required_universe


def _breadth_scale(*, breadth_ratio: float, breadth_scale_floor_ratio: float) -> float:
    if breadth_ratio <= breadth_scale_floor_ratio:
        return 0.0
    denominator = 1.0 - breadth_scale_floor_ratio
    if denominator <= 0:
        return 0.0
    return min(max((breadth_ratio - breadth_scale_floor_ratio) / denominator, 0.0), 1.0)


def summarize_trade_results(
    result: BreadthScaledAbsoluteMomentumCompositeResult,
    *,
    ...
    breadth_series = pd.Series([float(trade.speed3) for trade in trades], dtype="float64") if trades else pd.Series(dtype="float64")
    ...
    trades_per_rebalance = pd.Series(
        [len(group) for _, group in pd.DataFrame([trade.to_record() for trade in trades]).groupby("signal_confirm_date")],
        dtype="float64",
    ) if trades else pd.Series(dtype="float64")
    exposure_scale_pct = (trades_per_rebalance / float(top_k) * 100.0) if not trades_per_rebalance.empty else pd.Series(dtype="float64")
    return {
        ...
        "selected_rebalance_count": result.selected_rebalance_count,
        "skipped_for_scale_zero_rebalance_count": result.skipped_for_scale_zero_rebalance_count,
        ...
        "avg_regime_breadth_pct": round(float(breadth_series.mean()), 4) if not breadth_series.empty else None,
        ...
        "avg_target_positions": round(float(trades_per_rebalance.mean()), 4) if not trades_per_rebalance.empty else None,
        "avg_exposure_scale_pct": round(float(exposure_scale_pct.mean()), 4) if not exposure_scale_pct.empty else None,
        "rebalance_exit_count": sum(1 for trade in closed if trade.exit_reason == EXIT_REASON_REBALANCE),
    }


def run_breadth_scaled_absolute_momentum_composite_backtest(
    ...
    *,
    ...
    for signal_index in range(start_index, len(timeline) - 1, rebalance_interval):
        ...
        breadth_ratio = float((metrics_frame["momentum_pct"] >= breadth_momentum_floor_pct).mean())
        scale = _breadth_scale(breadth_ratio=breadth_ratio, breadth_scale_floor_ratio=breadth_scale_floor_ratio)
        if scale <= 0:
            ...
            trades.append(
                BreadthScaledAbsoluteMomentumCompositeTrade(
                    signal_id=(
                        ...
                        f"h{hold_bars}-am{min_momentum_pct}-bf{breadth_momentum_floor_pct}-bsf{breadth_scale_floor_ratio}-r{rank}"
                    ),
                    symbol=symbol,
