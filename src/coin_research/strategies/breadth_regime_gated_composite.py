...
DEFAULT_MIN_BREADTH_RATIO = 0.5
EXIT_REASON_REBALANCE = "rebalance_exit"


@dataclass(frozen=True)
class BreadthRegimeGatedCompositeTrade:
    signal_id: str
    symbol: str
    wave_start_date: Any
    wave_end_date: Any
...
@dataclass(frozen=True)
class BreadthRegimeGatedCompositeResult:
    trades: list[BreadthRegimeGatedCompositeTrade]
    evaluated_rebalance_count: int
    gated_rebalance_count: int
    skipped_for_breadth_rebalance_count: int
...
def _validate_params(
    *,
    ...
    if not 0.0 <= min_breadth_ratio <= 1.0:
        raise ValueError(f"min_breadth_ratio must be within [0, 1], got {min_breadth_ratio}")
    ...
    if breadth_momentum_floor_pct > min_momentum_pct:
        raise ValueError(
            "breadth_momentum_floor_pct should be <= min_momentum_pct so the regime filter is not stricter than the selection gate"
        )
    return required_universe


def summarize_trade_results(
    result: BreadthRegimeGatedCompositeResult,
    *,
    ...
    return {
        ...
        "gated_rebalance_count": result.gated_rebalance_count,
        "skipped_for_breadth_rebalance_count": result.skipped_for_breadth_rebalance_count,
...
def run_breadth_regime_gated_composite_backtest(
    ...
    *,
...
) -> BreadthRegimeGatedCompositeResult:
    ...
    if not prepared_frames:
        return BreadthRegimeGatedCompositeResult(
            trades=[],
            evaluated_rebalance_count=0,
            gated_rebalance_count=0,
            skipped_for_breadth_rebalance_count=0,
        )
    ...
    for signal_index in range(start_index, len(timeline) - 1, rebalance_interval):
        ...
        if breadth_ratio < min_breadth_ratio:
            skipped_for_breadth_rebalance_count += 1
            continue
        ...
        selected = ranking_frame.sort_values(["score", "momentum_pct"], ascending=False).head(top_k)
        if selected.empty:
            continue

        gated_rebalance_count += 1
        ...
        for rank, (symbol, row) in enumerate(selected.iterrows(), start=1):
            ...
            trades.append(
                BreadthRegimeGatedCompositeTrade(
                    signal_id=(
                        f"{symbol}-{_format_signal_time(signal_time)}-"
                        f"lb{lookback_bars}-vw{volatility_window}-top{top_k}-"
                        f"h{hold_bars}-am{min_momentum_pct}-bf{breadth_momentum_floor_pct}-br{min_breadth_ratio}-r{rank}"
                    ),
                    symbol=symbol,
    ...
    return BreadthRegimeGatedCompositeResult(
        ...
        gated_rebalance_count=gated_rebalance_count,
        skipped_for_breadth_rebalance_count=skipped_for_breadth_rebalance_count,
    )
