...
class MomentumVolatilityCompositeTrade:
    signal_id: str
    symbol: str
    wave_start_date: Any
    wave_end_date: Any
...
@dataclass(frozen=True)
class MomentumVolatilityCompositeResult:
    trades: list[MomentumVolatilityCompositeTrade]
...
def run_momentum_volatility_composite_backtest(
    ...
    *,
...
) -> MomentumVolatilityCompositeResult:
    ...
    if not prepared_frames:
        return MomentumVolatilityCompositeResult(trades=[])
    ...
    for signal_index in range(start_index, len(timeline) - 1, rebalance_interval):
        ...
        for rank, (symbol, row) in enumerate(selected.iterrows(), start=1):
            ...
            trades.append(
                MomentumVolatilityCompositeTrade(
                    signal_id=(
                        f"{symbol}-{_format_signal_time(signal_time)}-"
                        f"lb{lookback_bars}-vw{volatility_window}-top{top_k}-h{hold_bars}-r{rank}"
                    ),
                    symbol=symbol,
    ...
    return MomentumVolatilityCompositeResult(trades=trades)
