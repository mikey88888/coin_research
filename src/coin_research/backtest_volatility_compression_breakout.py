...
from .strategies.volatility_compression_breakout import (
...
STRATEGY_KEY = "volatility-compression-breakout"
TIMEFRAME_CHOICES = {"1d": "bar_time", "4h": "bar_time", "30m": "bar_time"}
ENGINE_CHOICES = {"account", "signal"}
EXIT_MODE = "squeeze_breakout_channel_or_time_stop"
...
def _run_id(
    *,
    ...
    return (
        f"{timestamp}__{timeframe}__sw{squeeze_window}_bw{breakout_window}_"
        f"ew{exit_window}_sq{_slug_float(squeeze_quantile)}_h{max_hold_bars}"
    )
...
def run_backtest(
    *,
    ...
    meta = {
        ...
        "strategy_label": "Volatility Compression Breakout",
        "engine_type": engine,
        "timeframe": timeframe,
        "exit_mode": EXIT_MODE,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest a volatility compression breakout prototype on crypto data")
    ...
    parser.add_argument("--squeeze-window", type=int, default=DEFAULT_SQUEEZE_WINDOW)
    ...
    parser.add_argument("--squeeze-quantile", type=float, default=DEFAULT_SQUEEZE_QUANTILE)
    ...
    trade_frame, summary, run_dir, _ = run_backtest(
        ...
        squeeze_window=args.squeeze_window,
